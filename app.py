...
# registrar devolucao
@app.route("/emprestimo/<id_emprestimo>/devolucao", methods=["PUT"])
def registrar_devolucao(id_emprestimo):
    try:
        ...
        mongo.db.emprestimos.update_one(
            {"_id": ObjectId(id_emprestimo)},
            {"$set": {"data_devolucao": datetime.now()}}
        )

        return {"sucesso": "Bicicleta devolvida com sucesso!"}, 200
    
    except Exception as erro:
        return {"erro": f"Erro ao registrar devolução: {str(erro)}"}, 400
